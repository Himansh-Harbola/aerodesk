...
begin
  ...
  where flight_id = p_new_flight_id
    and seat_number = old_seat.seat_number
  ...
  if not found then
    ...
    where flight_id = p_new_flight_id
      and is_available = true
    order by
      case class when old_seat.class then 0 when 'economy' then 1 when 'business' then 2 else 3 end,
      seat_number
    limit 1
    for update skip locked;
  end if;
  ...
  if not found then
    raise exception 'No seats are available on the new flight';
  end if;
